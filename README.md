...
![メイン画面と統計](docs/screenshot-hero.png)

- **ローカル完結**: データはすべて自分のPCの中(`data/focus.db` 1ファイル)。クラウドへの送信はありません
- **依存ゼロ**: Node.js だけで動きます(外部パッケージなし・ビルド不要)
- **スマホ対応**: [Tailscale](https://tailscale.com/) を使えばスマホからも同じ画面が使えます(オプション。PCだけでも全機能動作)
...
- Node.js **22.13 以上**(24 推奨)。外部パッケージは使っていないため、これだけで動きます
  - DBはNode組み込みの `node:sqlite` を使用(SQLiteファイル1個で完結)
- **Windows で Node.js を入れたくない場合**: [Releases](../../releases) の **Node同梱zip**(`-win-x64.zip`)なら、解凍して `Start.bat` をダブルクリックするだけで動きます(インストール不要。`runtime\` に Node.js 実行ファイルを同梱しています)。macOS 版の同梱zipはありません
...
> 新しい版への入れ替え方は **[アップデート(新しい版に入れ替える)](#アップデート新しい版に入れ替える)** をご覧ください。記録を引き継いだまま入れ替えられます。
...
- Windows をお使いの方 → [Windows での手順](#windows-での手順)
- Mac(または Linux)をお使いの方 → [macOS--linux-での手順](#macos--linux-での手順)

## Windows での手順

### 1. Node.js を用意する(初回だけ)

**`Setup.bat` をダブルクリック**してください。Node.js が入っているか、バージョンが足りているかを確認し、

- 入っていれば「セットアップは完了です!」と出ます
- 入っていない・古い場合は [nodejs.org](https://nodejs.org/ja) が自動で開きます。**「LTS」**版をダウンロードして、設定は変えずインストールし、もう一度 `Setup.bat` をダブルクリックしてください

> [Releases](../../releases) の **Node同梱zip**(`-win-x64.zip`)を使う場合、この手順は不要です。

### 2. 起動する

**`Start.bat` をダブルクリック**します。黒いコンソールが1つ開いたら起動しています。
ブラウザで **http://localhost:3000** を開いてください。

- 止めるときは、そのコンソールを閉じます
- 起動時のコンソールに、実際に待ち受けているURL(スマホから使うアドレスを含む)が出ます

### 3. サインイン時に自動起動する(任意)

**`RegisterAutostart.cmd` をダブルクリック**するだけです。サインイン時に自動で起動するようになります
(スタートアップフォルダにショートカットを作る方式。管理者権限は不要)。
やめるときは **`UnregisterAutostart.cmd`** をダブルクリックします。

<details>
<summary>別のやり方(タスクスケジューラ / pm2)</summary>
...
**Windows の手順はここまでです。** → [ここから先は共通](#ここから先は共通)

## macOS / Linux での手順

> **実機で確認できているのは macOS です**(macOS 26.5.1 / Apple Silicon)。
> Linux も同じ手順で動く見込みですが(OS固有の処理を使っていないため)、
> 実機での確認は取れていません。`Start.command` は `sh Start.command` でも起動できます。

### 1. Node.js を入れる(初回だけ)

**22.13 以上**が必要です。まずターミナルで確認してください。
...
> Node同梱zip は Windows 版のみです。macOS では Node.js のインストールが必要です。

### 2. 起動する

**`Start.command` をダブルクリック**します。ターミナルが開いて起動します。
ブラウザで **http://127.0.0.1:3000** を開いてください。

- 止めるときは、そのターミナルのウィンドウを閉じます
- 起動時の画面に、実際に待ち受けているURL(スマホから使うアドレスを含む)が出ます

### 3. 初回の「ブロックされました」を解除する
...
### 4. 「アクセス権がありません」と出たときだけ

**通常は何もいりません。** ZIP・`git clone` のどちらで入れても実行権限は付いています。
このメッセージが出たときだけ、次を実行してください。
...
**macOS の手順はここまでです。** → [ここから先は共通](#ここから先は共通)

---

# ここから先は共通

ここから下は Windows・macOS で同じです。
...
(**このアプリとは無関係の、別の会社の無料サービス**です。個人利用の無料枠があります)。
自分の端末どうしだけをつなぐVPNで、**社内LANやインターネットには公開されません**。

- 公式サイト: <https://tailscale.com/>(ダウンロードとアカウント作成)
- ソースコード: <https://github.com/tailscale/tailscale>(クライアントはオープンソース)

サーバーにするPCとスマホの両方に入れ、**同じアカウントでサインイン**すれば準備完了です。
Tailscale を入れない場合でも、そのPCのブラウザからは全機能が使えます。
...
**記録・設定はすべて `data/focus.db` の中にあります。** アプリの本体(プログラム)とデータは別のファイルなので、**データを持ったまま新しい版に入れ替えられます**。

> 入れ替えの前に、アプリの設定タブで **「💾 バックアップ作成」を1回押しておく**と安心です(万一のときは同じ画面から復元できます)。
...
`data/` と `backups/` は Git の管理対象外なので、**上書きされることはありません**。実行したらアプリを再起動して、ブラウザの画面をリロードしてください。

### ZIP(Download ZIP / Node同梱zip)で入れた場合

**古いフォルダは消さずに、新しいフォルダへ引き継ぐ**のが安全です。

1. **アプリを止める**(黒い窓 / ターミナルの窓を閉じる)
   - ⚠️動かしたままコピーすると、書きかけの記録(`data/focus.db-wal`)が取り残されて直前の記録が欠けることがあります
2. 新しい ZIP を展開する(古いフォルダとは**別の場所**に置きます)
3. **古いフォルダから新しいフォルダへ、次をコピー**します

   | コピーするもの | 中身 |
   |---|---|
   | `data/` フォルダごと | **記録・打刻・設定のすべて**(`focus.db` と `focus.db-wal` / `-shm` が入っていれば一緒に) |
   | `backups/` フォルダごと | 過去のバックアップ(不要なら省略可) |
   | `assets/` に**自分で足した**画像 | 自作キャラのフォルダ、季節イベント用の背景など |

   新しいフォルダに空の `data/` がある場合は、そのまま上書きして構いません。
4. 新しいフォルダの `Start.bat`(Mac は `Start.command`)で起動する
5. 画面を開いて、**設定タブの「🩺 データを点検」**で記録の件数が入れ替え前と同じかを確認する
6. 問題なければ古いフォルダを削除する

### 自動起動を登録している場合

登録は**フォルダの場所を覚えている**ので、**フォルダの場所が変わったら登録し直してください**。
...
- **設定(難易度・アラートの分数・キャラ)も引き継がれますか?** はい。設定も `data/focus.db` の中にあるので、`data/` をコピーすれば元のままです
- **バージョンはどこで分かりますか?** `package.json` の `version`、または [Releases](../../releases) のタグでご確認ください
- **古い版に戻したいときは?** 古いフォルダを残しておけば、そのまま起動できます(データは新しい版で記録したぶんが `data/` に入っているので、戻す場合は `backups/` から復元してください)
